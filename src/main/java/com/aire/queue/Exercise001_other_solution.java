...
// 打开转盘锁
public class Exercise001_other_solution {
...
//        String[] deadends = new String[]{"5557", "5553", "5575", "5535", "5755", "5355", "7555", "3555", "6655", "6455", "4655", "4455", "5665", "5445", "5645", "5465", "5566", "5544", "5564", "5546", "6565", "4545", "6545", "4565", "5656", "5454", "5654", "5456", "6556", "4554", "4556", "6554"};
        ...
        System.out.println(new Exercise001_other_solution().openLock(deadends, target));
//        System.out.println(System.currentTimeMillis() - time);
    ...
    public int openLock(String[] deadends, String target) {
        ...
        map2.put(targetVal, 0);
        Queue<Integer> queue2 = new LinkedList<>();
        queue2.add(targetVal);
        ...
        while (!queue1.isEmpty() && !queue2.isEmpty()) {
            if (queue1.size() < queue2.size()) {
                cengshu1++;
                boolean found = update(queue1, map1, map2, deadendsSet, targetVal, cengshu1);
                if (found) {
                    return cengshu1 + cengshu2;
                }
            } else {
                cengshu2++;
                boolean found = update(queue2, map2, map1, deadendsSet, targetVal, cengshu1);
                if (found) {
                    return cengshu1 + cengshu2;
                }
            }
    ...
    private boolean update(Queue<Integer> queue, Map<Integer, Integer> map1, Map<Integer, Integer> map2, Set<Integer> deadendsSet, int targetVal, int cengshu) {
        int queueSize = queue.size();
        for (int i = 0; i < queueSize; i++) {
            int val = queue.poll();
            boolean found = bfs(queue, map1, map2, deadendsSet, targetVal, val2arr(val), cengshu);
            if (found) {
                return true;
            }
        }
    ...
    private boolean bfs(Queue<Integer> queue, Map<Integer, Integer> map, Map<Integer, Integer> map2, Set<Integer> deadends, int target, int[] val, int cengshu) {
        ...
        for (int i = 0; i < vals.length; i++) {
            ...
            if (map2.containsKey(keyVal)) {
                // 这里能够找到说明了 两个队列相遇了
                return true;
            }
            map.put(keyVal, cengshu);
            if (target == keyVal) {
                return true;
            }
            queue.add(keyVal);
